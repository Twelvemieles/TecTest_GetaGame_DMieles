...
public class AIKart : MonoBehaviour {
    ...
    [Header ("Difficulty AI Variations")]

    [SerializeField]
    [Tooltip ("Velocity in Rect travel")]
    private float RectVelocity = 1f;
    [SerializeField]
    [Tooltip ("Velocity taking a curve")]
    private float CurveVelocity = 0.5f;
    [SerializeField]
    [Tooltip ("Velocity while drifting")]
    private float DriftVelocity = 0.7f;

    private enum State { Rect, Curve, Drift }
    private State state;
    ...
    public AIKart.Stats baseStats = new AIKart.Stats {
        TopSpeed = 10f,
        Acceleration = 5f,
    ...
    private void Start () {
        //TODO Take the child positions of Transform path and store it in node List
    ...
    private void ApplySteer () {
        ...
        //TODO Clamps value between -1 and 1 to steer vehicle taking the direction vector between position and next node
        float newSteer = (relativeVector.x / relativeVector.magnitude); //* maxSteerAngle;
        //TODO Select a acceleration depending on the stateMachine
        stateMachineSetter (newSteer);
        float accel = 0;
        switch (state) {
            case State.Rect:
                accel = RectVelocity;
        ...
        if (canMove) {
            MoveVehicle (accel, newSteer);
        }
    }
    void OnTriggerEnter (Collider other) {
        if (other.CompareTag ("Path") && other.gameObject != lastNode) {
            lastNode = other.gameObject;
            currentNode++;
        }
    }
    void stateMachineSetter (float steer) {
        //TODO Change states of the state machine depending on next Node
        if (Vector3.Distance (transform.position, nodes[currentNode].position) > 3 && Mathf.Abs (steer) < 0.3) {
            state = State.Rect;
        }
        if (Mathf.Abs (steer) > 0.3) {
            state = State.Curve;
        }
